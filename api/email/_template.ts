/**
 * HTML sanitization helper for template interpolations.
 */
...
/**
 * Pre-rendered static fragment cache for email templates.
 * Invariant header, footer, styles, and card wrappers are compiled once
 * and cached in memory across invocations.
 */
const CURRENT_YEAR = new Date().getFullYear();

const EMAIL_SHELL_HEADER = `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8">
    ...
    <tr>
      <td align="center">
        <p style="font-size: 12px; color: #64748b; margin: 0 0 4px 0;">
          © ${CURRENT_YEAR} Medstocksy Connect · Built for Modern Pharmacies
        </p>
        <p style="font-size: 11px; color: #94a3b8; margin: 0;">
          Intelligent Patient Relations &amp; Refill CRM
        </p>
      </td>
    </tr>
  </table>
...
export function getWelcomeEmailHtml(args: {
  ...
  const pharmacyName = escapeHtml(args.pharmacyName?.trim());
  const loginUrl = escapeHtml(args.loginUrl || 'https://connect.medstocksy.in');
    ...
    <tr>
      <td style="padding: 8px 4px;">
        ...
        <p style="font-size: 15px; line-height: 1.6; color: #334155; margin: 0 0 16px 0;">
          Welcome to <strong>Medstocksy Connect</strong>! Your pharmacy workspace ${pharmacyName ? `for "<strong>${pharmacyName}</strong>" ` : ''}is officially set up and ready to help you grow your pharmacy business and retain your patients.
        </p>
    ...
    <tr>
      <td style="padding: 8px 4px;">
        ...
        <div style="text-align: center; margin: 28px 0;">
          <a href="${resetUrl}" style="background-color: #6366f1; border-radius: 8px; color: #ffffff; font-size: 15px; font-weight: 600; text-decoration: none; text-align: center; display: inline-block; padding: 12px 28px; box-shadow: 0 2px 4px rgba(99, 102, 241, 0.25);">
            Reset My Password
          </a>
        </div>
